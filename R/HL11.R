#Updated by Lars 2013 Sept 27
HL11 <- function(fv, w, Z, family) {
  n <- nrow(Z)
  p <- ncol(Z)
  W1 <- diag(w[1:n]^2)
  W2 <- diag(w[-(1:n)]^2)
  ...
  if (family$family[1] == "gamma" ) V.prime <- rep(2, length(mu))
  ...
  dW1.dmu <- -2/(g.prime^3)*g.bis/V - 1/(V^2)*V.prime*(1/g.prime^2)
